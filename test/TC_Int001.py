...
class TC_Int001(Node):
    def __init__(self):
        super().__init__('TC_Int001')
...
def test_TC_Int001(test_node):
    # Test step 1: Activate driving mode
    ...
    # Test step 2: Simulate driving state by publishing odometry and path
    ...
    assert wait_for_condition(
        ...
        timeout=5.0
    ), "Vehicle didn't start moving"
    print("Expected Result 2: PASSED")  # ← Expected Result 2
     
     
    # Test step 3: Simulate a close obstacle
    test_node.publish_obstacle(0.3)

    assert wait_for_condition(
        lambda: test_node.obstacle_detected is True,
        timeout=5.0
    ), f"Obstacle not detected in time: {test_node.obstacle_detected}"
    print("Expected Result 3a: PASSED")  # ← Expected Result 3a

    assert wait_for_condition(
        lambda: test_node.last_cmd is not None and test_node.last_cmd.speed == 0.0,
        timeout=5.0
    ), f"Vehicle didn't stop: {test_node.last_cmd}"
